...
ax1.set_title(f"Graft: {graft_diameter}mm × {graft_length}mm")
ax1.axis('off')
...
with col_b:
if st.button(f"Edit F{i+1}", key=f"edit_{i}"):
st.session_state[f'editing_{i}'] = True
with col_c:
if st.button(f"Delete F{i+1}", key=f"del_{i}"):
...
Ensure printer is set to "Actual Size" or "100%" scale
Use standard A4/Letter paper
Sterilize printed template before use
Wrap template around actual graft
Use holes as cutting guides """)
Download template

if st.session_state.fenestrations:
# Create downloadable template
buf = io.BytesIO()
fig2.savefig(buf, format='pdf', bbox_inches='tight', dpi=300)
buf.seek(0)
